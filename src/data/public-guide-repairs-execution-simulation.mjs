const slug = 'simulyatsiya-ispolneniya-kripto-strategiy';

const content = String.raw`
<article class="article-page public-repair-page" data-public-guide-repair="execution-simulation-research-boundary-r1" data-repair-state="BOUNDED_PUBLIC_COPY_REPAIRED_REVERIFY_REMAINS">
  <div class="container public-repair-container">
    <header class="public-repair-header">
      ...
      <p class="public-repair-lead">Execution simulation помогает проверять, насколько результаты стратегии зависят от идеализированных fills, очереди, задержек и издержек. Но исторический replay не доказывает собственный market impact и не превращает research candidate в готовую live-реализацию. Каждое допущение должно быть явно задано, откалибровано и проверено на независимых наблюдениях.</p>
    </header>
    ...
    <section class="public-repair-card" aria-labelledby="es-boundary">
      ...
      <p>Эта страница описывает методологию исследования execution realism. Она не является production blueprint, архитектурным приказом, runtime-конфигурацией или торговым разрешением. Прохождение симулятора не доказывает прибыльность, currentness, безопасность или готовность стратегии к live.</p>
    </section>

    <section class="public-repair-card" aria-labelledby="es-replay">
      <h2 id="es-replay">2. Что исторический replay может и не может доказать</h2>
      <ul>
        <li><strong>Partial fills и queue position:</strong> при агрегированных L2-данных реальная позиция собственного лимитного ордера обычно неизвестна, поэтому fill logic является модельным допущением.</li>
    ...
    <section class="public-repair-card" aria-labelledby="es-models">
      ...
      <table class="public-repair-table">
        ...
        <tbody>
          <tr><td>Queue model</td><td>MODEL_ASSUMPTION</td><td>Сравнить несколько моделей с наблюдаемыми fill/cancel результатами; не считать одну функцию или exponent универсальными.</td></tr>
          <tr><td>Market impact</td><td>EMPIRICAL_CANDIDATE_MODEL</td><td>Проверять sensitivity к размеру, liquidity, volatility и режиму. Square-root form может быть baseline, но не гарантией.</td></tr>
          ...
          <tr><td>Latency model</td><td>PATH_SPECIFIC_INPUT</td><td>Предпочитать measured/historical distributions или stress scenarios вместо фиксированной константы.</td></tr>
        </tbody>
      </table>
    </section>
    ...
    <section class="public-repair-card" aria-labelledby="es-validation">
      <h2 id="es-validation">5. Как калибровать против независимых наблюдений</h2>
      <ol>
        ...
        <li>Измерять implementation shortfall, fill-rate drift, adverse-fill diagnostics и другие заранее определённые discrepancies.</li>
        <li><strong>Калибровать против независимых paper/live observations</strong>, не используя live discrepancy для скрытого post-hoc подбора OOS результата.</li>
    ...
    <section class="public-repair-card" aria-labelledby="es-promotion">
      ...
      <p><strong>Здесь нет live execution или capital authority.</strong> Страница не задаёт entry, exit, leverage, position size, deployment, cloud migration, venue selection или движение капитала.</p>
    </section>
...
export const executionSimulationRepairs = [
  {
    ...
    repairState: 'BOUNDED_PUBLIC_COPY_REPAIRED_REVERIFY_REMAINS',
    content,
  },
];
